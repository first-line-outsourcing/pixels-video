...
$._PPP_={
    ...
    // var event = new CSEvent("com.adobe.PProPanel", "APPLICATION");
		app.setExtensionPersistent("com.adobe.SlideDeckPanel", 0); // 0, while testing (to enable rapid reload); 1 for "Never unload me, even when not visible."
	},
	...
	searchForBinWithName : function (nameToFind) {
		// deep-getPackages a folder by name in project
  ...
  importFilesFromPaths: function (filePaths) {
    var proj = app.project;
    if (proj) {
      // We have an array of File objects; importFiles() takes an array of paths.
      return proj.importFiles(filePaths);
    }

    return;
  },
  getProjectBin: function() {
    return app.project.rootItem.children[0].getMediaPath();
  },

  getSelectedItemsPaths: function() {
    var viewIDs = app.getProjectViewIDs();
    var viewSelection = app.getProjectViewSelection(viewIDs[viewIDs.length-1]);
    var array = [];
    for (var i = 0; i < viewSelection.length; i++) {
      array[i] = viewSelection[i].getMediaPath();
    }
    return array;
  },
      ...
      if (markers) {
        ...
        if (numMarkers > 0) {
          for (var current_marker = markers.getFirstMarker();
               current_marker !== undefined;
               current_marker = markers.getNextMarker(current_marker)) {
            markerList.push(current_marker)
          }
        }
      }
    } else {
  ...
  addVideoTrack: function() {
    if (app.project.activeSequence.videoTracks[0].clips.numItems > 0) {
      app.enableQE();
  ...
  addSubClips: function(projectItemSourcePath, startTimeSeconds, endTimeSeconds, markerStart, segmentNum) {
    var matchingPaths = app.project.rootItem.findItemsMatchingMediaPath(projectItemSourcePath, 1);
    var hasHardBoundaries = 0;
    var takeVideo = 1;
    var takeAudio = 1;
    var projectItem = matchingPaths[0];
    if (projectItem) {
      if ((projectItem.type == ProjectItemType.CLIP) || (projectItem.type == ProjectItemType.FILE)) {
        var newSubClipName = "ClipSegment-" + segmentNum;
        var newSubClip = projectItem.createSubClip(newSubClipName,
          startTimeSeconds,
          endTimeSeconds,
          hasHardBoundaries,
          takeVideo,
          takeAudio);
        var lastVideoTrack = app.project.activeSequence.videoTracks[0];
        lastVideoTrack.overwriteClip(newSubClip, markerStart);
        var clipsBin = $._PPP_.getClipsBin('Segments');
        newSubClip.moveBin(clipsBin);
      }
      else {
        $._PPP_.updateEventPanel("Could not sub-clip " + projectItem.name + ".");
      }
  ...
  addFrameHoldSubClips: function(projectItemSourcePath, startTimeSeconds, endTimeSeconds, segmentNum, frameHoldSegNum) {
    var matchingPaths = app.project.rootItem.findItemsMatchingMediaPath(projectItemSourcePath, 1);
    var hasHardBoundaries = 0;
    var takeVideo = 1;
    var takeAudio = 1;
    ...
    if (projectItem) {
      if ((projectItem.type == ProjectItemType.CLIP) || (projectItem.type == ProjectItemType.FILE)) {
        var newSubClipName = "ClipSegment-" + segmentNum + "-" + "FrameHold-" + frameHoldSegNum;
        var newSubClip = projectItem.createSubClip(newSubClipName,
          startTimeSeconds,
          endTimeSeconds,
          hasHardBoundaries,
          ...
          takeAudio);
        var lastVideoTrack = app.project.activeSequence.videoTracks[0];
        var lastClip = lastVideoTrack.clips[(lastVideoTrack.clips.numItems - 1)];
        lastVideoTrack.insertClip(newSubClip, lastClip.end.seconds);
        var clipsBin = $._PPP_.getClipsBin('Segments');
        newSubClip.moveBin(clipsBin);
      }
      else {
  ...
  deleteMarkers: function() {
    ...
    if (activeSequence) {
      ...
      if (markers) {
        var numMarkers = markers.numMarkers;
        if (numMarkers > 0) {
          for (var current_marker = markers.getLastMarker();
               current_marker !== undefined;
               current_marker = markers.getLastMarker()) {
  ...
  getClipsBin: function(binName) {
    var targetBin;
    var rootBin = app.project.rootItem;
    for (var i = 0; i < rootBin.children.numItems; i++) {
      if (rootBin.children[i].type === 2 && rootBin.children[i].name === binName) {
        targetBin = rootBin.children[i];
      }
    }
    if (!targetBin) {
  ...
  deleteVideoTrack: function(){
    if(app.project.activeSequence.videoTracks[0].clips.numItems === 0){
      app.enableQE();
      var mySeq = qe.project.getActiveSequence();
      mySeq.removeVideoTrack(0);
      mySeq.removeAudioTrack(0);
    }
  },
  ...
  createMarkers: function() {
    ...
    if (activeSequence) {
      var markers = activeSequence.markers;
      for (var i = 0; i < 23; i++) {
        var newCommentMarker = markers.createMarker(i * 10);
        newCommentMarker.name = 'Segment-' + i;
        newCommentMarker.end = i * 10 + 10;
      }
    } else {
      $._PPP_.updateEventPanel("No active sequence.");
    }
  },
  getAllProjectSequences: function(){
    var sequencesList = [];
    var sequencesNum = app.project.sequences.numSequences;
    for(var i = 0; i < sequencesNum; i++){
      sequencesList.push(app.project.sequences[i])
    }
    return JSON.stringify(sequencesList);
  },

  getSelectedSequence: function(idSeq) {
    for(var i = 0; i < sequencesNum; i++){
      if(app.project.sequences[i].sequenceID===idSeq){
